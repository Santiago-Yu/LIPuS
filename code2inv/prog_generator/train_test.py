...
def loop_tester():
    ...
    if cmd_args.phase == 'test' and len(dataset.test_indices) == len(stat_counter.reported):
        sys.exit()
...
if __name__ == '__main__':
    ...
    for epoch in range(cmd_args.num_epochs):
        ...
        if cmd_args.phase == 'train' or not cmd_args.tune_test:
            loop_tester()
        if cmd_args.phase == 'test' and cmd_args.tune_test == 0:
            continue
        acc_reward = 0.0
        pbar = tqdm(range(100))
        for k in pbar:
            if cmd_args.phase == 'test' and len(stat_counter.reported) == len(dataset.test_indices):
                sys.exit()
