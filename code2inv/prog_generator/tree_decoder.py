...
from code2inv.coldstart.cold_starter import parseIntoAction
...
class IDecoder(nn.Module):
    ...
    def choose_action(self, state, cls_w, use_random, eps, expr=False):
        ...
        #self.varNumber = len(self.ruleset["var"])
        if expr and (len(self.usedVarsId) >= cmd_args.maxLinearVarNumber - 1 or self.varUsedId >= self.varNumber - 1):
            picked = torch.tensor(0)
        elif expr and cmd_args.linearcomplete:
            if self.varUsedId >= self.varNumber - 1:
                picked = torch.tensor(0)
            else:
                picked = torch.tensor(1)
        elif self.cold_start is not None and self.cold_start:
            # Going to create a label
            picked = torch.tensor([self.cold_start_queue.popleft()])
        elif use_random:
...
def genExprTree(ruleset : dict, rule : str, ind = -1): # expand those has no choice.
    rules = ruleset[rule]
...
class InvariantTreeNode(ExprNode):
    def __init__(self, pg_node, rule = None):
        ...
        self.expanded = None
        self.smthistory = []
        self.solvePossible = True
        self.islinearArg = False
    ...
    def to_expr_node(self, program_graph : ProgramGraph):
        ...
        infix = []
        for p in expr_str:
            if p in Z3_CMP or p in Z3_OP or p in ("&&", "||") or p in program_graph.const_nodes or p in program_graph.raw_variable_nodes:
                infix.append(p)
            else:
                infix = infix + list(p)
        postfixexpr = []
            ...
            if self.rule == "p":
                ...
                for child in self.children:
                    if child.rule is None or child.rule == "":
                        continue
                    elif child.rule == "p" and len(child.children) > 0 and child.children[0].name != 'var':
...
class GeneralDecoder(AssertAwareDecoder):
    ...
    def __init__(self, latent_dim, graph_sample, coldstart_templetes):
        ...
        # Use Rule Set, Program Graph, to generate action queque.
        if cmd_args.cold_start:
            self.cold_start_queue = parseIntoAction(coldstart_templetes, graph_sample)
            self.cold_start = cmd_args.cold_start
            self.cold_start_tried = 0
        #
        self.varUsedId = -1
        self.varNumber = 0
        self.usedVarsId = set()
    ...
    def forward(self, env, node_embedding, use_random, eps=0.05):
        ...
        self.varUsedId = 0
        self.varNumber = env.num_vars()
        self.usedVarsId.clear()
    ...
    def updateTreeWRTNode(self, env, node, node_embedding, use_random, eps, depth=0):
        ...
        if node.name == "" and node.rule is not None and len(node.children) == 0 and node.rule != "var":
            ...
            ind = self.choose_action(self.latent_state, w, use_random, eps, expr= (node.rule =="expr"))
    ...
    def choose_var(self, env, node_embedding, use_random, eps, var_list):
        ...
        var_list_notUsed = var_list
        if not cmd_args.linearcomplete:
            var_list_notUsed = []
            for varer in var_list:
                if varer not in self.usedVarsId:
                    var_list_notUsed.append(varer)

        selector = node_embedding[var_list_notUsed, :]

        if self.varUsedId != -1 and cmd_args.linearcomplete and self.varNumber <= cmd_args.maxLinearVarNumber:
            picked = torch.tensor(self.varUsedId).view(-1)
            self.varUsedId += 1
        else:
            picked = self.choose_action(self.latent_state, selector, use_random, eps)
            self.varUsedId += 1
        ...
        idx = to_num(picked)
        idx = var_list_notUsed[idx]
        env.update_used_core_var(env.pg_nodes()[idx])
        self.usedVarsId.add(idx)
