...
from code2inv.templeter.const_smt_helper import const_smt_solve, const_smt_save, smt_check
...
class CounterExample(object):
    ...
    def recursivefindvar(self, expr_root, varnames):
        if expr_root.rule == 'var':
            if expr_root.name not in varnames:
                varnames.add(expr_root.name)
                return 1
            else:
                return 0
        else:
            res = 0
            for child in expr_root.children:
                res += self.recursivefindvar(child, varnames)
            return res


    def getVarN(self, expr_root):
        varnames = set()
        counter = self.recursivefindvar(expr_root, varnames)
        return counter

    def check(self, expr_root):
        if self.kind == "pre":
            # if assignment is not full, we leave it good. put it elsewhere.
            # if len(self.config) < self.getVarN(expr_root):
            #     return "good"

            # we should get a SAT result for positive example
            if self.helper(self.config, expr_root) is None or self.helper(self.config, expr_root):
                return "good"
            else:
                return "bad"

        elif self.kind == "post":
            # if assignment is not full, we leave it good.
            # if len(self.config) < self.getVarN(expr_root):
            #     return "good"

            # we should get an UNSAT result for negative example
            if self.helper(self.config, expr_root) is None or not self.helper(self.config, expr_root):
                return "good"
            else:
                return "bad"


        elif self.kind == "loop": # This one has great problem. should be both false or both true.

            # if assignment is not full, we leave it good.
            # lnt = self.getVarN(expr_root)
            # if len(self.config[0]) < lnt\
            #         or len(self.config[1]) < lnt:
            #     return "good"


            t1 = self.helper(self.config[0], expr_root)
            t2 = self.helper(self.config[1], expr_root)

            if t1 is None or t2 is None or t1 is False or (t1 is True and t2 is True):
                return "good"
            else:
                return "false"
...
class CEHolder(object):
    ...
    def eval(self, key, expr_root):
        ...
        if cmd_args.debug == True:
            print("counters ", samples)
            print("counter numbers: ", len(samples))
            # if len(samples) == 11 and key == "T:":
            #     print("Herereere!!")
        # DEBUG

        if cmd_args.unsat_core is None or not cmd_args.unsat_core: # Use the old checking
            # for ce in samples:
            #     if ce.check(expr_root) == "good":
            #         s += 1.0
            # if len(samples) - s < 0.1: # directly pass, give a good one.
            if smt_check(samples, key, expr_root):
                # DEBUG
                if cmd_args.debug == True:
                    print("one shot goal on ", str(key))
                # DEBUG

                # still have to put these samples into histories.
                const_smt_save(samples, key, expr_root)

                return 1.4

        # return the good proportion
        # DEBUG
        if cmd_args.debug == True:
            print("has to const solving ", str(key))
        # DEBUG
        s_ave = const_smt_solve(samples, key, expr_root)
        # DEBUG
        if cmd_args.debug == True:
            if s_ave == 0:
                print("cannot solving.... roll back:")
            else:
                print("exp after solving:", str(expr_root))
        # DEBUG
        return s_ave
...
def reward_0(holder, lambda_holder_eval, lambda_new_ce, scores):
    try:
        ...
        if key == 'T:':
            ...
            scores[2] *= 0.49
            if cmd_args.inv_reward_type == 'ordered':
                scores[0] = scores[1] = 1.4

        if key is not None:
            if cmd_args.debug:
                print("get counter example::    ", key, ce)
...
def boogie_result(g, expr_root):
    ...
    if res > 0:
        ...
        r = stat_counter.report_str(g.sample_index)
        tqdm.write(r)
